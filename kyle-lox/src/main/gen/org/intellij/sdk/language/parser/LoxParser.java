...
public class LoxParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // item_*
  static boolean loxFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "loxFile")) return false;
    while (true) {
      int c = current_position_(b);
      if (!item_(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "loxFile", c)) break;
    }
    return true;
  }
